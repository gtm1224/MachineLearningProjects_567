...
class QLearningAgent:
    ...
    def reset(self) -> None:
        ...
        """
        ### YOUR CODE HERE ###
        self.q_table = np.random.randn(self.state_space.n, self.action_space.n)
        ### END OF YOUR CODE ###
    ...
    def act(self, state: int, exploit: bool = False) -> int:
        ...
        """
        ### YOUR CODE HERE ###
        if exploit or np.random.rand() > self.explore_rate:
    ...
    def learn(self) -> dict:
        ...
        s_next = batch.next_states


        max_q_next = np.max(self.q_table[s_next], axis=1)

        target = batch.rewards + self.discount * max_q_next
        old_q = self.q_table[batch.states, batch.actions]
        td_error = target - old_q
        self.q_table[batch.states, batch.actions] += self.lr * td_error
        ### END OF YOUR CODE ###
        return {
            'td_error': td_error,
